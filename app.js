(() => {
  ...
  function load(){
    try{
      const saved = JSON.parse(localStorage.getItem('itqanNabdStage1V3')||'{}');
      return {...defaults,...saved,student:{...defaults.student,...(saved.student||{}),mastery:{...defaultMastery,...((saved.student||{}).mastery||{})}}};
    }catch{return structuredClone(defaults)}
  }
  let state=load();
  const save=()=>localStorage.setItem('itqanNabdStage1V3',JSON.stringify(state));
  ...
  function renderHome(){
    root.innerHTML=`<section class="hero">
      <div class="hero-copy"><span class="eyebrow">مرتبطة بمنهج اللغة العربية • 2025–2026</span><h1>إتقان <span>| نبض</span></h1><p>منصة يومية تحول المنهج إلى رحلة قصيرة قابلة للقياس: <b>قراءة، مهارة لغوية، كتابة، وتحدي استقلال</b>. النقاط تحفّز الطالبة، بينما تحتفظ المنصة بمؤشرات المهارة لتساعد المعلمة على اتخاذ قرار تدريسي أدق.</p><div class="hero-actions"><button class="btn primary" id="goStudent" type="button">دخول الطالبة</button><button class="btn secondary" id="goTeacher" type="button">لوحة المعلمة</button></div><div class="tag-row" style="margin-top:18px"><span class="tag skill">6 وحدات فعلية</span><span class="tag">30 نقطة يوميًا</span><span class="tag">المهارة لا الدرجة فقط</span><span class="tag">منهج ← درس ← مهمة ← أثر</span></div></div>
      <div class="hero-media"><div class="mascot-frame"><img src="assets/salama-reading.jpg" alt="سلامة بالمريول الرصاصي تقرأ كتابًا"/></div><div class="companion-card"><span class="companion-icon">⭐</span><div><b>سلامة ترافق الطالبة</b><br>في رحلة واضحة بلا ازدحام بصري: أفهم ← أتدرّب ← أؤدي وحدي.</div></div></div>
    </section>`;
