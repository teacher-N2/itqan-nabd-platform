window.ITQAN_SERVER = (() => {
  ...
  const teacherDataUrl = `${url}/functions/v1/itqan-teacher-data`;
  const resetPinUrl = `${url}/functions/v1/itqan-reset-pin`;
  ...
  async function teacherData(action = 'dashboard') {
    await refreshIfNeeded();
    const r = await fetch(teacherDataUrl, {
      method: 'POST', headers: authHeaders(), body: JSON.stringify({ action })
    });
    const data = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(data?.message || 'تعذر تحديث بيانات لوحة المعلمة.');
    return data;
  }
  ...
  async function resetTeacherPin(resetCode, newPin) {
    const r = await fetch(resetPinUrl, {
      method: 'POST', headers: baseHeaders(),
      body: JSON.stringify({ reset_code: String(resetCode || '').trim(), new_pin: String(newPin || '') })
    });
    const data = await r.json().catch(() => ({}));
    if (!r.ok) {
      const messages = {
        invalid_fields: 'أدخلي رمز إعادة التعيين ورقمًا سريًا من أربعة أرقام أو أكثر.',
        invalid_reset_code: 'رمز إعادة التعيين غير صحيح أو استُخدم سابقًا.',
        teacher_not_found: 'تعذر العثور على حساب المعلمة.',
        reset_failed: 'تعذر تغيير الرقم السري.'
      };
      throw new Error(messages[data.error] || data.message || 'تعذر إعادة تعيين الرقم السري.');
    }
    return data;
  }
  ...
  restore();
  return { url, key, apiUrl, teacherDataUrl, resetPinUrl, signIn, signOut, restore, select, teacherData, api, bootstrapTeacher, resetTeacherPin, syncEncryptedBank, get session() { return session; } };
})();
